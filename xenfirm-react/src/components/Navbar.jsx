...
const navLinks = [
  ...
  { 
    label: 'Products', 
    dropdown: [
      { to: '/pages/upskill', label: 'XenFirm Upskill' },
      { to: '#', label: 'NowSuite', comingSoon: true },
      { to: '#', label: 'Linkora', comingSoon: true },
    ]
  },
  { 
    label: 'Services', 
    dropdown: [
      { to: '/pages/services', label: 'Web Engineering' },
      { to: '/pages/services', label: 'Mobile Applications' },
      { to: '/pages/services', label: 'AI & SaaS Solutions' },
      { to: '/pages/services', label: 'Digital Marketing' },
      { to: '/pages/services', label: 'UI/UX Design' },
      { to: '/pages/services', label: 'Cloud & DevOps' },
      { to: '/pages/services', label: 'Security Audits' },
      { to: '/pages/services', label: 'Analytics Setup' },
      { to: '/pages/services', label: 'E-Commerce' },
      { to: '/pages/services', label: 'Brand Identity' },
      { to: '/pages/services', label: 'SaaS Architecture' },
      { to: '/pages/services', label: '24/7 Support' },
    ]
  },
...
const Navbar = () => {
  ...
  const [activeDropdown, setActiveDropdown] = useState(null); // Stores the label of the open mobile dropdown
  ...
  return (
    <>
      <nav className={`navbar${scrolled ? ' scrolled' : ''}`} id="navbar">
        <div className="nav-inner">
          ...
          <ul className="nav-menu">
            {navLinks.map(l => (
              <li key={l.label} className={l.dropdown ? 'nav-dropdown' : ''}>
                {l.dropdown ? (
                  <>
                    ...
                    <div className="dropdown-menu">
                      {l.dropdown.map(item => (
                        <Link 
                          key={item.label} 
                          to={item.to} 
                          className={`dropdown-item ${item.comingSoon ? 'coming-soon' : ''}`}
                        >
                          {item.label}
      ...
      <div className={`mobile-menu${mobileOpen ? ' open' : ''}`} id="mobileMenu">
        {navLinks.map(l => (
          l.dropdown ? (
            <div key={l.label} className="mobile-dropdown">
              <div className="mobile-dropdown-header" onClick={() => setActiveDropdown(activeDropdown === l.label ? null : l.label)}>
                {l.label}
                <svg width="12" height="8" viewBox="0 0 12 8" fill="none" style={{ transform: activeDropdown === l.label ? 'rotate(180deg)' : 'none', transition: '0.3s' }}>
                ...
                <div className="mobile-dropdown-items">
                  {l.dropdown.map(item => (
                    <NavLink 
                      key={item.label} 
                      to={item.to} 
                      className={item.comingSoon ? 'coming-soon' : ''}
            ...
            <NavLink key={l.to || l.label} to={l.to} end={l.end} onClick={() => setMobileOpen(false)}>
              {l.label}
            </NavLink>
          )
        ))}
      ...
      {/* WhatsApp FAB */}
      <a href="https://wa.me/919080824626?text=Hi%20Xenfirm%2C%20I%27d%20like%20to%20discuss%20a%20project." target="_blank" rel="noopener" className="whatsapp-fab" title="Chat on WhatsApp">
